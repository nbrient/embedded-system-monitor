...
static const cyaml_strval_t logTargetStrings[] = {
    {"journalctl",LOG_TARGET_JOURNALCTL},{"LOG_TARGET_JOURNALCTL",LOG_TARGET_JOURNALCTL},
    {"syslog",LOG_TARGET_SYSLOG},{"LOG_TARGET_SYSLOG",LOG_TARGET_SYSLOG},
};
...
static const cyaml_schema_field_t collectorCfgSchema[] = {
    ...
    CYAML_FIELD_UINT("timer",CYAML_FLAG_DEFAULT,CollectorCfg,intervalSec),CYAML_FIELD_END
};
/** @brief CYAML field schema for the generic_config mapping. */
static const cyaml_schema_field_t genericCfgSchema[] = {
    CYAML_FIELD_ENUM("log_method",CYAML_FLAG_DEFAULT,MonitorCfg,logTarget,logTargetStrings,CYAML_ARRAY_LEN(logTargetStrings)),CYAML_FIELD_END
};
...
static const cyaml_schema_value_t rootSchema={CYAML_VALUE_MAPPING(CYAML_FLAG_POINTER,MonitorCfg,monitorCfgSchema)};
/** @brief libcyaml runtime configuration. */
static const cyaml_config_t cyamlCfg={.log_fn=cyaml_log,.mem_fn=cyaml_mem,.log_level=CYAML_LOG_WARNING};
...
extern void config_use_default_path(void){snprintf(configFilePath,sizeof(configFilePath),"%s%s",DEFAULT_CONFIG_DIR,CONFIG_FILENAME);}
extern MonitorCfg config_load(void){
    int8_t returnErr=0; MonitorCfg result,*parsed=NULL; apply_defaults(&result);
    returnErr=cyaml_load_file(configFilePath,&cyamlCfg,&rootSchema,(void**)&parsed,NULL);
    if(returnErr!=CYAML_OK){fprintf(stderr,"[config] %s — using defaults\n",cyaml_strerror(returnErr));return result;}
    result=*parsed; cyaml_free(&cyamlCfg,&rootSchema,parsed,0); return result;
}
