...
static const cyaml_schema_field_t collectorCfgSchema[] = {
    CYAML_FIELD_UINT("enable_acquisition", CYAML_FLAG_DEFAULT, CollectorCfg,
                     enabled),
...
static const cyaml_schema_field_t genericCfgSchema[] = {
    CYAML_FIELD_ENUM("log_method", CYAML_FLAG_DEFAULT, MonitorCfg, logTarget,
                     logTargetStrings, CYAML_ARRAY_LEN(logTargetStrings)),
    CYAML_FIELD_END};
/** @brief CYAML field schema for the top-level MonitorCfg mapping. */
static const cyaml_schema_field_t monitorCfgSchema[] = {
    CYAML_FIELD_MAPPING("generic_config", CYAML_FLAG_DEFAULT, MonitorCfg,
                        logTarget, genericCfgSchema),
    CYAML_FIELD_MAPPING("cpu_config", CYAML_FLAG_DEFAULT, MonitorCfg, cpu,
                        collectorCfgSchema),
    CYAML_FIELD_MAPPING("mem_config", CYAML_FLAG_DEFAULT, MonitorCfg, mem,
                        collectorCfgSchema),
    CYAML_FIELD_MAPPING("irq_config", CYAML_FLAG_DEFAULT, MonitorCfg, irq,
                        collectorCfgSchema),
    CYAML_FIELD_MAPPING("proc_config", CYAML_FLAG_DEFAULT, MonitorCfg, proc,
                        collectorCfgSchema),
    CYAML_FIELD_END};
...
static void apply_defaults(MonitorCfg *cfg) {
  ...
  cfg->mem.intervalSec = 5;
  cfg->irq.enabled = true;
  cfg->irq.intervalSec = 5;
  cfg->proc.enabled = true;
  cfg->proc.intervalSec = 5;
}
extern void config_set_path(const char *path) {
  ...
  if (len > 5 && strcmp(path + len - 5, ".yaml") == 0) {
    ...
    while (dlen > 0 && dir[dlen - 1] == '/')
      dir[--dlen] = '\0';
    snprintf(configFilePath, sizeof(configFilePath), "%s/%s", dir,
             CONFIG_FILENAME);
  }
  TRACE("config path: %s\n", configFilePath);
}
extern void config_use_default_path(void) {
  snprintf(configFilePath, sizeof(configFilePath), "%s%s", DEFAULT_CONFIG_DIR,
           CONFIG_FILENAME);
}
extern MonitorCfg config_load(void) {
  int8_t returnErr = 0;
  MonitorCfg result, *parsed = NULL;
  apply_defaults(&result);
  returnErr = cyaml_load_file(configFilePath, &cyamlCfg, &rootSchema,
                              (void **)&parsed, NULL);
