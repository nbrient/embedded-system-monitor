 ...
 *
 * Responsibilities:
 * - Parse optional CLI config path.
 * - Load MonitorCfg via config_load().
 * - Initialise the sink and active collectors.
 * - Block until SIGINT or stdin 'q'.
 * - Cleanly stop and destroy all collectors.
 *
 * @version 1.0
 */
...
/**
 * @brief SIGINT handler — sets shutdownFlag and wakes the main thread.
 *
 * @param _  Signal number, not used.
 */
 ...
 *
 * Calls exit(EXIT_FAILURE) if sigaction fails.
 */
...
int main(int argc, char *argv[]) {
    ...
    set_up_signals();
    sink_init(cfg.logTarget);

    returnErr = cpu_collector_new(cfg.cpu);
    CHECK_ERR(returnErr < 0, "[Main] Error during cpu_collector_new");
...
static void set_up_signals(void) {
    ...
    returnErr = sigaction(SIGINT, &sa, NULL);
