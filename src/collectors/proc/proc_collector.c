...
extern int8_t proc_collector_new(CollectorCfg cfg) {
  ...
  if (returnErr >= 0) {
    ...
    CHECK_ERR(returnErr < 0,
              "[ProcCollector] Error when initialising the mutex");
  }
...
extern int8_t proc_collector_ask_start(void) {
  ...
  CHECK_ERR(returnErr < 0,
            "[ProcCollector] Error when creating the worker thread");
      ...
      if (fgets(line, sizeof(line), fileToRead)) {
        ...
        int matched =
            sscanf(line,
                   ...
                   "%*d %*d %u %*d %u "
                   "%*u %*u %u %u",
                   &pid, comm, &state, &ppid, &ut, &st, &pri, &nth, &vsz, &rss);
...
static int8_t set_up_mq(void) {
  ...
  if (procMq < 0) {
    CHECK_ERR(true, "[ProcCollector] Fail to open the mqueue");
    returnErr = -1;
  }
  return returnErr;
}
...
static int8_t tear_down_mq(void) {
  ...
  if (returnErr >= 0) {
    returnErr = mq_close(procMq);
    CHECK_ERR(returnErr < 0, "[ProcCollector] Error closing mqueue");
  } else {
    CHECK_ERR(true, "[ProcCollector] Error unlinking mqueue");
  }
  return returnErr;
}
...
static int8_t send_msg_mq(MqMsgProcCollector *msg) {
  int8_t returnErr =
      mq_send(procMq, (char *)msg, sizeof(MqMsgProcCollector), 0);
...
static int8_t read_msg_mq(MqMsgProcCollector *dest) {
  int8_t returnErr =
      mq_receive(procMq, (char *)dest, sizeof(MqMsgProcCollector), NULL);
...
static int8_t perform_collect(void) {
  ...
  CHECK_ERR(update_proc_data() < 0,
            "[ProcCollector] Error updating process data");
...
static int8_t perform_stop(void) {
  ...
  if (returnErr < 0) {
    ...
    CHECK_ERR(returnErr < 0,
              "[ProcCollector] Error when joining the worker thread");
  }
