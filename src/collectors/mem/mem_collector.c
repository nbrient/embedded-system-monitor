...
typedef struct {
  StateMemCollector
      destinationState;      /**< State to enter after this transition. */
...
/**
 * @brief Map one /proc/meminfo key-value pair to the corresponding MemStats
 * field.
 *
...
extern int8_t mem_collector_new(CollectorCfg cfg) {
  ...
  if (returnErr >= 0) {
    ...
    CHECK_ERR(returnErr < 0,
              "[MemCollector] Error when initialising the mutex");
  }
...
extern int8_t mem_collector_ask_start(void) {
  ...
  CHECK_ERR(returnErr < 0,
            "[MemCollector] Error when creating the worker thread");
...
#define M(f, m)                                                                \
  ...
  M("MemTotal", memTotal)
  M("MemFree", memFree) M("MemAvailable", memAvailable) M("Buffers", buffers)
      M("Cached", cached) M("SwapCached", swapCached) M("Active", active)
          M("Inactive", inactive) M("SwapTotal", swapTotal)
              M("SwapFree", swapFree) M("Dirty", dirty)
                  M("Writeback", writeback) M("Shmem", shmem) M("Slab", slab)
                      M("SReclaimable", sReclaimable)
                          M("SUnreclaim", sUnreclaim)
                              M("KernelStack", kernelStack)
                                  M("PageTables", pageTables)
                                      M("VmallocTotal", vmallocTotal)
                                          M("VmallocUsed", vmallocUsed)
                                              M("CommitLimit", commitLimit)
                                                  M("Committed_AS", committed)
#undef M
}
...
static int8_t perform_action(ActionMemCollector action) {
  ...
  case A_NONE:
    break;
  case A_COLLECT:
    returnErr = perform_collect();
    break;
  case A_STOP:
    returnErr = perform_stop();
    break;
  default:
    break;
  }
...
static int8_t perform_stop(void) {
  ...
  if (returnErr < 0) {
    ...
    CHECK_ERR(returnErr < 0,
              "[MemCollector] Error when joining the worker thread");
  }
