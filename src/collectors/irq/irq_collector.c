...
extern int8_t irq_collector_new(CollectorCfg cfg) {
  ...
  if (returnErr >= 0) {
    ...
    CHECK_ERR(returnErr < 0,
              "[IrqCollector] Error when initialising the mutex");
  }
...
extern int8_t irq_collector_ask_start(void) {
  ...
  CHECK_ERR(returnErr < 0,
            "[IrqCollector] Error when creating the worker thread");
...
static int8_t set_up_mq(void) {
  ...
  if (irqMq < 0) {
    CHECK_ERR(true, "[IrqCollector] Fail to open the mqueue");
    returnErr = -1;
  }
  return returnErr;
}
...
static int8_t tear_down_mq(void) {
  ...
  if (returnErr >= 0) {
    returnErr = mq_close(irqMq);
    CHECK_ERR(returnErr < 0, "[IrqCollector] Error closing mqueue");
  } else {
    CHECK_ERR(true, "[IrqCollector] Error unlinking mqueue");
  }
  return returnErr;
}
...
static int8_t read_msg_mq(MqMsgIrqCollector *dest) {
  int8_t returnErr =
      mq_receive(irqMq, (char *)dest, sizeof(MqMsgIrqCollector), NULL);
...
static int8_t perform_stop(void) {
  ...
  if (returnErr < 0) {
    ...
    CHECK_ERR(returnErr < 0,
              "[IrqCollector] Error when joining the worker thread");
  }
