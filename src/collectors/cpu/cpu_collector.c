...
extern int8_t cpu_collector_new(CollectorCfg cfg) {
  ...
  if (returnErr >= 0) {
    ...
    CHECK_ERR(returnErr < 0,
              "[CpuCollector] Error when initialising the mutex");
  }
...
extern int8_t cpu_collector_ask_start(void) {
  ...
  CHECK_ERR(returnErr < 0,
            "[CpuCollector] Error when creating the worker thread");
...
static int8_t update_load_average(void) {
  ...
  if (fileToRead == NULL) {
    ...
    if (fgets(line, sizeof(line), fileToRead)) {
      sscanf(line, "%f %f %f %d/%d %15s", &cpuMetrics.load1min,
             &cpuMetrics.load5min, &cpuMetrics.load15min,
...
static int8_t update_cpu_stat(void) {
  ...
  if (fileToRead == NULL) {
    ...
    while (fgets(line, sizeof(line), fileToRead)) {
      ...
      if (sscanf(line, "%15s %u %u %u %u %u %u %u", tag, &ct.user, &ct.nice,
                 &ct.system, &ct.idle, &ct.iowait, &ct.irq, &ct.softirq) < 8)
        continue;
...
static int8_t perform_collect(void) {
  ...
  CHECK_ERR(update_load_average() < 0,
            "[CpuCollector] Error updating load average");
...
static int8_t perform_stop(void) {
  ...
  if (returnErr < 0) {
    ...
    CHECK_ERR(returnErr < 0,
              "[CpuCollector] Error when joining the worker thread");
  }
